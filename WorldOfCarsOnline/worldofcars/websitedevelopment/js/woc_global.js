...
function onPlayClick() {
	if( jq( '#play_button' ).hasClass( 'play_active' ) ) {
		...
		if( window.location == PATH.siteRoot +'/welcome/' || window.location == PATH.siteRoot +'/welcome' ) {
			launchGame();
		} else {
			moduleSpawning == 'registration';
			loadRegMod();
		}
	}
}
...
function loadPurchMod(){
	...
	jq('#purchDiv').css({
		...
		swfobject.embedSWF(PATH.swfSecure +'/purchasing/DVC_OS.swf', "flashPurch", "100%", "100%", "9.0.0","http://a.dolimg.com/swf/dcom/expressInstall.swf", purchFlashVars, purchFlashParams, attributes, function(e){
...
function loadAcctMgr(){
    ...
    var attributes = {
		...
		swfobject.embedSWF(PATH.swfSecure +'/acm/DVC_OS.swf', "flashACM", "100%", "100%", "10.0.0","http://a.dolimg.com/swf/dcom/expressInstall.swf", flashvars, flashparams, attributes, function(e){
